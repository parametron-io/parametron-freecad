...
| Cross-repository Engine → FreeCAD rehearsal | Completed external rehearsal, real FreeCAD | `parametron-engine` branch `test/real-freecad-target-mutation-rehearsal`; final independent Stage 2 audit for [FreeCAD #9](https://github.com/parametron-io/parametron-freecad/issues/9) | Separate from the repository-local native fixture proof above. From a clean rehearsal HEAD rebased onto corrected Engine main, real FreeCAD 1.1.1 proved suppress/unsuppress, hide/unhide, safe deletion, unsafe-delete native rejection, representative `post_mutation_validity` failure, target-state observation, runtime-result return, rich schema-1 traversal, Engine verification, normal record/package emission, exact raw-evidence provenance, and representative repeatability. All six active Engine ↔ FreeCAD JSON runtime surfaces were schema 1.0, the schema-2 active-path audit was `0 / 0 / 0`, and the independent audit returned `READY TO CLOSE FREECAD #9`. This row records cross-repository closure evidence; it is not repository-local automated coverage. |
